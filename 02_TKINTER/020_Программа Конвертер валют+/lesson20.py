# coding=utf-8
#######################################################
# 020_Программа Конвертер валют (lesson20)
# python 3.x
...
#================================
root = Tk()
root.title('Privatbank (UAH) - for Cards')
...
START_AMOUNT = 1000                         # SUMA (default)
...
# USD course (ряд 1)
usd_currency = Label(header_frame, text="EUR", font="Arial 10")
...
eur_currency = Label(header_frame, text="USD", bg="#ccc", font="Arial 10")
...
e_uah.insert(0, START_AMOUNT)   # подставить сумму по умолчанию
...
# s.configure('red.TButton', padding=5, backgound="red")       # только для одного (T)Element (red)
# btn_calc = ttk.Button(calc_frame, text="Обмен", command=exchange, style="red.TButton")  # ttk style

btn_calc.grid(row=1, column=1, columnspan=2, sticky=EW, padx=10)    # разместить в таблице grid
...
e_usd.insert(0, round(START_AMOUNT / float(JSON_object[0]['sale']), 2))     # конвертация USD
